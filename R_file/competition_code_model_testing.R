...
head(h2o.varimp(gbm_model),20)

m2 = train(y ~ OverallQual + GarageArea + BsmtFinSF1 + LotArea + X1stFlrSF + X2ndFlrSF + FullBath + MasVnrArea + Fireplaces + YearRemodAdd,data=d, method = "lm", trControl = ctrl, metric = "Rsquared")
summary(m2)
m3 = train(y ~ OverallQual + GarageArea + BsmtFinSF1 + LotArea + X1stFlrSF + X2ndFlrSF, data=d, method = "lm", trControl = ctrl, metric = "Rsquared")
summary(m3)

m4 = train(y ~ OverallQual + GarageArea + BsmtFinSF1 + LotArea + X1stFlrSF + X2ndFlrSF + FullBath + MasVnrArea + Fireplaces + YearRemodAdd,data=d, method = "gbm", trControl = ctrl, metric = "Rsquared",tuneGrid = expand.grid(
